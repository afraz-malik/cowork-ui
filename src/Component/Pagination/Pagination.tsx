        ...
        <div className='tableNumber'>
          <Dropdown className='paginationDropdown' onSelect={handleSelect}>
            <Dropdown.Toggle id='pageDropDown'>
              {selectedValue !== null
                ? selectedValue
                : limitDivided.length > 0 && limitDivided[0]}
            </Dropdown.Toggle>
          ...
          <p>
            {generateSequence(page)} -{' '}
            {(page && page - 1) === 0 ? '' : page && page - 1}
            {allRequestList.length} of {totalValue} {paginationTitle}
          </p>
        </div>
        <div className='paginationNumber'>
          <button
            onClick={() => prevPage()}
            className={prevButton === true ? '' : 'disablePag'}
          >
            <FontAwesomeIcon icon={faArrowLeft} /> Previous
          </button>
          {paginationDivided.map((number) => (
            <button key={`page` + number} onClick={() => setPage(number)}>
              {number}
            </button>
          ))}
          <button
            onClick={() => nextPage()}
            className={nextButton === true ? '' : 'disablePag'}
          >
