...
export const CustomHeader = ({
  ...
  <div>
    ...
    <div className='calenderBtn'>
      <button onClick={() => handleYesterdayClick(setDueDate)}>
        Yesterday
      </button>
...
export function multiplyBySixAndShowSeries(originalValue: number) {
  ...
  const resultArray = Array.from(
    { length: Math.ceil(originalValue / 10) },
    (_, index) => {
...
export const messageFormatDate = (dateString: string) => {
  ...
  const daysDifference: any = Math.floor(
...
export function findTimeGap(startTime: any, endTime: any) {
  ...
  const startIndex = timeOptions.findIndex(
    (option: any) => option.value === startTimeValue
  )
  const endIndex = timeOptions.findIndex(
    (option: any) => option.value === endTimeValue
  )
