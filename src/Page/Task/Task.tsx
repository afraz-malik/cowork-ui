  ...
  useEffect(() => {
    getTaskList("PENDING", sort).then((data) => {
      const updatedData = data.map((task: any) => {
        if (task.assigned_member_data) {
          task.assigned_member_data = JSON.parse(task.assigned_member_data);
        }
        return task;
      });

      setPendingList(updatedData);
    });
    ...
    getTaskList("DONE", sort).then((data) => {
      ...
      setDoneList(updatedData);
    });
  }, [show, deleteShow, taskEditShow, sort, taskShow]);
                                  ...
                                  {(provided: any) => (
                                    ...
                                    >
                                      ...
                                      <div className="taskMember">
                                        {task.assigned_member_data.length > 0 &&
                                          task.assigned_member_data.map(
