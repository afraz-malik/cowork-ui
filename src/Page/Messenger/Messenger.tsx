...
import {
  ...
  CHAT_TEXT,
  GROUP_ICON,
} from '../../CommonFunction/constant'
import { getChatHistory, getChatters } from '../../api/message'
import {
  convertTimeToReadable,
  enhanceGroupUserName,
  enhanceToGroupUserName,
  formattedDateForChatHeadWithCurrent,
  fromNow,
  messageFormatDate,
...
const Messenger = () => {
  ...
  const closeDropdown = (e: MouseEvent) => {
    if (
      ...
      !dropdownContent.current.contains(e.target as Node)
    ) {
  ...
  const updateContacts = async () => {
    ...
    if (!!company) {
      ...
      const tusers = tempUsers.filter(
        (e: any, i: number) =>
          e.email !== storage.user.email && !chattersEmail.includes(e.email)
      )
      setToUsers(tusers)
    }
  }
  ...
  const handleToReceiveSocket = async (payload: any) => {
    const single =
      chatter && !payload.group_id && chatter.recipient === payload.sender
    const group =
      chatter && payload.group_id && chatter.group_id === payload.group_id
    if (single || group) {
        ...
        <div className='chat'>
          <div className='contacts'>
            ...
            <div className='contact'>
              {showDropdown ? (
                <div
                  className='contact1 chatter-select'
                  onClick={() => handleFromSet(null)}
                >
              ...
              {users.map((e, i) => (
                <div
                  className={
                    'chatList ' +
                    ((!e.group_id && e.recipient === chatter?.recipient) ||
                    (e.group_id && e.group_id === chatter?.group_id)
                      ? 'chatter-select'
                      : '')
                  }
                  key={i}
                >
                  {/* <>{e.group_id}</> */}
                  <div className='chatBox' onClick={() => handleFromSet(e)}>
                    ...
                    <div className='text'>
                      <div className='bogdan-krivenchenko'>
                        {e.group_id ? enhanceGroupUserName(e.name!) : e.name}
                      </div>
                      <div className='hi-how-are'>
                        {e.message?.length > 25
                          ? `${e.message.substring(0, 25)}...`
                          : e.message}
                      </div>
                    </div>
                  </div>
                  <div className='parent'>
                    <div className='div16'>
                      {messageFormatDate(e.created_at)}
                    </div>
          ...
          <div className='chat1'>
                    ...
                    <div className='user'>
                      <div className='avatar-parent6'>
                        ...
                        <div className='dropdown'>
                          <div className='sarah-kline'>
                            {e.name && e.role === 'group'
                              ? enhanceToGroupUserName(e.name)
                              : e.name}
                          </div>
            ...
            <div className='chat2' ref={chatContainerRef}>
              <div className='friday-january-26th-parent'>
                ...
                {chatter ? (
                  messages.map((e, i) => {
                    return e.sender === currentUser.email ? (
                      <ToMessage
                        key={i}
                        {...e}
                        online_status={true}
                      ...
                      <FromMessage
                        key={i}
                        {...e}
                        online_status={chatter?.online_status}
